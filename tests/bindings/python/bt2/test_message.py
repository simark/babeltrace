...
class AllMessagesTestCase(unittest.TestCase):
    ...
    def test_all_msg_with_cc(self):
        params = {'with_cc': True}
        self._src_comp = self._graph.add_source_component(self._src, 'my_source', params)
        ...
        for i, msg in enumerate(self._msg_iter):
            if i == 0:
                ...
                self.assertEqual(msg.default_clock_snapshot.cycles, i)
            elif i == 2:
                ...
                self.assertEqual(msg.beginning_default_clock_snapshot.cycles, i)
                self.assertEqual(msg.end_default_clock_snapshot.cycles, i)
            elif i == 6:
    ...
    def test_all_msg_without_cc(self):
        ...
        for i, msg in enumerate(self._msg_iter):
            if i == 0:
                ...
                self.assertIsNone(msg.beginning_default_clock_snapshot)
                self.assertIsNone(msg.end_default_clock_snapshot)
            elif i == 5:
                self.assertIsInstance(msg, bt2.message._PacketEndMessage)
                self.assertEqual(msg.packet.addr, self._packet.addr)
            elif i == 6:
