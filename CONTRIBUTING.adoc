...
+
To undo all environment changes, run `bt-deactivate`. Note that
sourcing the `env.sh` file of another build automatically deactivates the
previous one first, so you don't need to run `bt-deactivate` manually
...
. Runs it with `--list-tests -v quiet --order decl` to discover
  individual Catch2 test cases.
