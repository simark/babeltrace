...
use `tests/utils/python/vendor/pytest-6.1.2` for Python{nbsp}3.5 and
Python{nbsp}3.6 or `tests/utils/python/vendor/pytest-7.4.3` for
Python{nbsp}3.7+.

Running the {bt2} tests requires Python, the `bt2` Python bindings, and
{bt2} Python plugin support.

=== Run tests with Make

The easiest way to run all the available {bt2} tests is:

----
$ make check
----

You may pass extra pytest arguments with the
`BT_TESTS_PYTEST_EXTRA_ARGS` environment variable, for example to
run them in parallel using
https://pypi.org/project/pytest-xdist/[pytest-xdist] and get a TAP
output using https://pypi.org/project/pytest-tap/[pytest-tap]:

----
$ make check BT_TESTS_PYTEST_EXTRA_ARGS='-nauto --tap'
----
...
. Source `tests/utils/env.sh`, generated in your build directory.
+
This script sets up the required environment variables (library search
paths, AddressSanitizer options, in-tree pytest path if applicable,
Python module search path, Babeltrace plugin path, and more).
+
It also defines the following shell functions:
+
`bt-pytest`::
    Wraps `pytest` to use the configured Python interpreter
    (`$BT_TESTS_PYTHON_BIN`) and adds an option to set the pytest cache
    directory to be within the build directory.

`bt-python`::
    Calls `bt-python-env` with the configured Python interpreter
    (`$BT_TESTS_PYTHON_BIN`), forwarding any extra arguments.

`bt-python-env`::
    Runs a given command (``"$@"``) with the `LD_PRELOAD` and
    `ASAN_OPTIONS` environment variables needed for sanitizers
    (ASan/UBSan) to work correctly with Python.
+
Example:
+
----
$ bt-python-env gdb --args "$BT_TESTS_PYTHON_BIN" -m pytest -sv \
                tests/bindings/python/bt2/test_clock_snapshot.py
----

. Run `bt-pytest` or `pytest` in the _source_ directory (that's where
  all the test files are).
+
Use `bt-pytest` for any build. For in-tree builds, you may use `pytest`
as is.

[NOTE]
====
You can also use `tests/utils/run-in-env.sh` directly without
sourcing `tests/utils/env.sh`, for example:

----
$ /path/to/build/tests/utils/run-in-env.sh bt-pytest tests/
----
====

For example, with an out-of-tree build:

----
$ source /path/to/build/tests/utils/env.sh
$ cd /path/to/source
$ bt-pytest tests/
----
...
See the https://docs.pytest.org/en/latest/how-to/usage.html[pytest documentation]
to learn about the various ways to run specific tests. Here are a few
examples:

Run all the tests of a test file:: {empty}
+
----
$ bt-pytest tests/bindings/python/bt2/test_value.py
----

Run a specific test class:: {empty}
+
----
$ bt-pytest tests/bindings/python/bt2/test_value.py::TestRealValue
----

Run a specific tests:: {empty}
+
----
$ bt-pytest tests/plugins/sink.ctf.fs/test_stream_names.py::test_metadata_file_exists
$ bt-pytest tests/cpp-common/test-uuid.cpp::test_bt2c_uuid_size_returns_16
----

=== Main `conftest.py`

`tests/conftest.py` is the main pytest configuration file.

Its role is to:

* Set configuration attributes (paths).

* Collect and run {cpp} tests using
  https://github.com/catchorg/Catch2[Catch2].

* Define common fixtures and markers.

==== Catch2 test collection

`conftest.py` implements a custom pytest collector for {cpp} tests.

pytest collects any `test-*.cpp` file under `tests/` as a Catch2 test
file. The collector:

. Finds the corresponding compiled test binary in the build directory.

. Runs it with `--list-test-names-only` to discover individual Catch2
  test cases.

. Creates a pytest item for each Catch2 test case.

This makes it possible to run, filter, and report {cpp} tests like any
other pytest test.

==== Configuration attributes

`conftest.py` sets the following attributes on the pytest configuration
object (`request.config`) and may be accessed by tests or fixtures:

`build_root_dir` (`pathlib.Path`)::
    Root build directory.

`build_tests_dir` (`pathlib.Path`)::
    `tests` build directory.

`src_tests_dir` (`pathlib.Path`)::
    `tests` source directory.

`ctf_traces_dir` (`pathlib.Path`)::
    CTF test traces directory (`tests/data/ctf-traces`).

==== Common fixtures

The following session-scoped fixtures are available to all tests
and fixtures:

Path fixtures::
    `src_root_dir`, `src_tests_dir`, `build_root_dir`,
    `build_tests_dir`, `common_data_dir`, and `ctf_traces_dir`.

Component class fixtures::
    `muxer_comp_cls`, `sink_ctf_comp_cls`, `src_ctf_comp_cls`,
    `trimmer_comp_cls`, `details_comp_cls`, `pretty_comp_cls`,
    `dmesg_comp_cls`, and `dummy_comp_cls`.

Other::
    `os_type`: current OS type.

==== Markers

`needs_lttng_utils_plugin`::
    Skip the test if the `lttng-utils` plugin isn't available.
+
`tests/plugins/flt.lttng-utils.debug-info/conftest.py`
automatically adds this marker to all its test items.

=== `bt_tests_utils` module

`tests/utils/python/bt_tests_utils.py` is a common testing utilities
module. Any test or `conftest.py` file may import it; our convention is
to import it as `btu`:
...
OS type::
    `OsType` enumeration and `os_type()` function.

Subprocess execution::
    `run()` runs a subprocess with proper environment setup
    (`PYTHONPATH` and the rest).

CLI parameter formatting::
    `cli_params_from_obj()` formats a Python object as a `babeltrace2`
    CLI `--params` value string.

Graph execution::
    `SinkComponentSpec` (sink component specification) and `convert()`
    (runs a graph to completion with source, filter, and sink component
    specifications, like `bt2.TraceCollectionMessageIterator`).

Output comparison test::
    `convert_sink_text_details_test()` runs a graph with
    `sink.text.details` and compares the output to an expectation.
+
Supports the `BT_TESTS_WRITE_EXPECTED` environment variable to write
the expected file from the actual output.

Path resolution::
    `build_dir_of_source_file()` returns the build directory equivalent
    of a source file path.

Event collection::
    `tcmi_events()` collects and returns all event messages from a
    `bt2.TraceCollectionMessageIterator`.

Component class lookup::
    `CompClsType` enumeration and `find_comp_cls_in_path()` finds a
    component class within a plugin found by searching a given path.

pytest{nbsp}6/7 compatibility::
    `PytestFile`, `PytestItem`, and `install_pytest_collect_file()`
    provide a consistent API across pytest versions for custom
    collectors.

=== `bt_tests_cli_utils` module

`tests/utils/python/bt_tests_cli_utils.py` is a common CLI testing
utilities module. Any test or `conftest.py` file may import it; our
convention is to import it as `btu_cli`:
...
`CliParams`::
    Component initialization or query parameters as a Python dictionary.

`cli_params_from_obj()`::
    Formats a Python object as a `babeltrace2` CLI `--params` option
    value string.

`run_cli()`::
    Runs the built `babeltrace2` CLI with custom
    arguments `args`, which can contain both strings and `CliParams`
    instances, considering specific plugin paths.

`run_cli_sink_text_details_test()`::
    Runs a graph with `sink.text.details` and compares the output to
    some expectation string/path.
+
Supports the `BT_TESTS_WRITE_EXPECTED` environment variable to write
the expected file from the actual output.

In general, prefer `btu.convert()` and
`btu.convert_sink_text_details_test()` over `btu_cli.run_cli()` and
`btu_cli.run_cli_sink_text_details_test()`.

=== Write tests

Follow standard pytest best practices, for example (for Python tests):

* Name test files `test_*.py`, test classes `Test*`, and test
  functions/methods `test_*()`.

* Use test classes to group related tests if needed; otherwise use
  free functions within the test file.

* Use fixtures for setup/teardown and shared resources.
+
Make sure to correctly scope your fixtures.

* Use `pytest.raises()` to test expected exceptions.

* Use `@pytest.mark.parametrize()` to run a test with different inputs.

* Use `@pytest.mark.skip()` or `pytest.mark.skipif()` to skip tests.

* Keep tests focused: one logical assertion per test when practical.

Refer to existing tests in the `tests` directory for examples and
patterns specific to the {bt2} project.
...
The {bt2} project is configured to use the {cpp17} standard.

{cpp17} makes it possible to build {bt2} with a broad range of
compilers, from GCC{nbsp}7.5 and Clang{nbsp}5.

IMPORTANT: The project is currently moving from {cpp}11 to {cpp}17,
therefore some parts of the text below may not be accurate.
...
`c-string-view.hpp`::
    ...
    C{nbsp}string.
+
We have this because `std::string_view` doesn't imply null termination,
...
==== `bt2s`: drop-in replacements of {cpp}20 STL features
...
use `std::invoke()` with an immediately invoked function expression
(lambda) to perform complex initialization of an ideally `const`
variable:
+
[source,cpp]
----
const auto size = std::invoke([this] {
...
* Always use `std::invoke()` to call an immediately invoked function
...
method/function or `std::invoke()` with an immediately invoked function
expression (lambda):
+
[source,cpp]
----
