...
struct trace_ir_metadata_maps *trace_ir_metadata_maps_create(struct trace_ir_maps *ir_maps,
                                                             const bt_trace_class *in_trace_class);

void trace_ir_metadata_maps_destroy(struct trace_ir_metadata_maps *md_maps);

bt_stream_class *
trace_ir_mapping_create_new_mapped_stream_class(struct trace_ir_maps *ir_maps,
                                                const bt_stream_class *in_stream_class);

bt_stream_class *
trace_ir_mapping_borrow_mapped_stream_class(struct trace_ir_maps *ir_maps,
                                            const bt_stream_class *in_stream_class);

bt_stream *trace_ir_mapping_create_new_mapped_stream(struct trace_ir_maps *ir_maps,
...
bt_event_class *
trace_ir_mapping_create_new_mapped_event_class(struct trace_ir_maps *ir_maps,
                                               const bt_event_class *in_event_class);

bt_event_class *trace_ir_mapping_borrow_mapped_event_class(struct trace_ir_maps *ir_maps,
                                                           const bt_event_class *in_event_class);

bt_packet *trace_ir_mapping_create_new_mapped_packet(struct trace_ir_maps *ir_maps,
...
static inline struct trace_ir_data_maps *
borrow_data_maps_from_input_trace(struct trace_ir_maps *ir_maps, const bt_trace *in_trace)
{
    BT_ASSERT(ir_maps);
    BT_ASSERT(in_trace);

    struct trace_ir_data_maps *d_maps = static_cast<trace_ir_data_maps *>(
...
static inline struct trace_ir_data_maps *
borrow_data_maps_from_input_stream(struct trace_ir_maps *ir_maps, const bt_stream *in_stream)
{
    BT_ASSERT(ir_maps);
    BT_ASSERT(in_stream);

    return borrow_data_maps_from_input_trace(ir_maps, bt_stream_borrow_trace_const(in_stream));
}

static inline struct trace_ir_data_maps *
borrow_data_maps_from_input_packet(struct trace_ir_maps *ir_maps, const bt_packet *in_packet)
{
    BT_ASSERT(ir_maps);
    BT_ASSERT(in_packet);

    return borrow_data_maps_from_input_stream(ir_maps, bt_packet_borrow_stream_const(in_packet));
}

static inline struct trace_ir_metadata_maps *
borrow_metadata_maps_from_input_trace_class(struct trace_ir_maps *ir_maps,
                                            const bt_trace_class *in_trace_class)
{
    BT_ASSERT(ir_maps);
    BT_ASSERT(in_trace_class);

    struct trace_ir_metadata_maps *md_maps = static_cast<trace_ir_metadata_maps *>(
        g_hash_table_lookup(ir_maps->metadata_maps, (gpointer) in_trace_class));
    if (!md_maps) {
        md_maps = trace_ir_metadata_maps_create(ir_maps, in_trace_class);
        g_hash_table_insert(ir_maps->metadata_maps, (gpointer) in_trace_class, md_maps);
    }
...
static inline struct trace_ir_metadata_maps *
borrow_metadata_maps_from_input_stream_class(struct trace_ir_maps *ir_maps,
                                             const bt_stream_class *in_stream_class)
{
    BT_ASSERT(in_stream_class);

    return borrow_metadata_maps_from_input_trace_class(
        ir_maps, bt_stream_class_borrow_trace_class_const(in_stream_class));
}

static inline struct trace_ir_metadata_maps *
borrow_metadata_maps_from_input_event_class(struct trace_ir_maps *ir_maps,
                                            const bt_event_class *in_event_class)
{
    BT_ASSERT(in_event_class);

    return borrow_metadata_maps_from_input_stream_class(
        ir_maps, bt_event_class_borrow_stream_class_const(in_event_class));
}
