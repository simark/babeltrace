    ...
    for (ch = str; *ch != '\0'; ch++) {
        ...
        if (uch < 32 || uch >= 127) {
            ...
            case '\v':
                ...
                break;
            default:
                g_string_append_printf(ctx->tsdl, "\\x%02x", (unsigned int) uch);
                break;
            }
        } else if (*ch == '"' || *ch == '\\') {
            g_string_append_c(ctx->tsdl, '\\');
...
{
    g_string_append_printf(ctx->tsdl, "integer { size = %u; align = %u;", size, alignment);
    ...
    if (mapped_clock_class_name) {
        g_string_append_printf(ctx->tsdl, " map = clock.%s.value;", mapped_clock_class_name);
    }
                                    ...
                                    __attribute__((unused)) struct fs_sink_ctf_field_class_bool *fc)
{
     ...
     */
    append_integer_field_class_from_props(ctx, fc->base.size, fc->base.base.alignment, false,
                                         ...
                                         struct fs_sink_ctf_field_class_bit_array *fc)
{
     ...
     * hexadecimal base.
     */
    append_integer_field_class_from_props(ctx, fc->size, fc->base.alignment, false,
                                       ...
                                       struct fs_sink_ctf_field_class_int *fc)
{
    ...
    bool is_signed = bt_field_class_type_is(type, BT_FIELD_CLASS_TYPE_SIGNED_INTEGER);

    if (bt_field_class_type_is(type, BT_FIELD_CLASS_TYPE_ENUMERATION)) {
        g_string_append(ctx->tsdl, "enum : ");
    }

    append_integer_field_class_from_props(ctx, fc->base.size, fc->base.base.alignment, is_signed,
    ...
    if (bt_field_class_type_is(type, BT_FIELD_CLASS_TYPE_ENUMERATION)) {
        ...
        for (i = 0; i < bt_field_class_enumeration_get_mapping_count(ir_fc); i++) {
            ...
            if (is_signed) {
                s_mapping =
                    bt_field_class_enumeration_signed_borrow_mapping_by_index_const(ir_fc, i);
                mapping = bt_field_class_enumeration_signed_mapping_as_mapping_const(s_mapping);
                s_ranges = bt_field_class_enumeration_signed_mapping_borrow_ranges_const(s_mapping);
                ranges = bt_integer_range_set_signed_as_range_set_const(s_ranges);
            } else {
                u_mapping =
                    bt_field_class_enumeration_unsigned_borrow_mapping_by_index_const(ir_fc, i);
                mapping = bt_field_class_enumeration_unsigned_mapping_as_mapping_const(u_mapping);
                u_ranges =
                    bt_field_class_enumeration_unsigned_mapping_borrow_ranges_const(u_mapping);
                ranges = bt_integer_range_set_unsigned_as_range_set_const(u_ranges);
            }

            label = bt_field_class_enumeration_mapping_get_label(mapping);
            range_count = bt_integer_range_set_get_range_count(ranges);
            ...
            for (range_i = 0; range_i < range_count; range_i++) {
                ...
                if (is_signed) {
                    ...
                    range =
                        bt_integer_range_set_signed_borrow_range_by_index_const(s_ranges, range_i);
                    lower = bt_integer_range_signed_get_lower(range);
                    upper = bt_integer_range_signed_get_upper(range);

                    if (lower == upper) {
                        g_string_append_printf(ctx->tsdl, "%" PRId64, lower);
                    } else {
                        g_string_append_printf(ctx->tsdl, "%" PRId64 " ... %" PRId64, lower, upper);
                    }
                } else {
                    const bt_integer_range_unsigned *range;
                    uint64_t lower, upper;

                    range = bt_integer_range_set_unsigned_borrow_range_by_index_const(u_ranges,
                                                                                      range_i);
                    lower = bt_integer_range_unsigned_get_lower(range);
                    upper = bt_integer_range_unsigned_get_upper(range);

                    if (lower == upper) {
                        g_string_append_printf(ctx->tsdl, "%" PRIu64, lower);
                    } else {
                        g_string_append_printf(ctx->tsdl, "%" PRIu64 " ... %" PRIu64, lower, upper);
                    }
                }
                                     ...
                                     struct fs_sink_ctf_field_class_float *fc)
{
    ...
    g_string_append_printf(ctx->tsdl, "floating_point { mant_dig = %u; exp_dig = %u; align = %u; }",
                          ...
                          struct fs_sink_ctf_field_class *fc)
{
    ...
    while (fc->type == FS_SINK_CTF_FIELD_CLASS_TYPE_ARRAY ||
        ...
        if (fc->type == FS_SINK_CTF_FIELD_CLASS_TYPE_ARRAY) {
            struct fs_sink_ctf_field_class_array *array_fc = fs_sink_ctf_field_class_as_array(fc);

            g_string_append_printf(lengths, "[%" PRIu64 "]", array_fc->length);
            fc = array_fc->base.elem_fc;
        } else {
            struct fs_sink_ctf_field_class_sequence *seq_fc =
                fs_sink_ctf_field_class_as_sequence(fc);

            g_string_append_printf(lengths, "[%s]", seq_fc->length_ref->str);
                                              ...
                                              struct fs_sink_ctf_field_class_struct *struct_fc)
{
    ...
    for (i = 0; i < struct_fc->members->len; i++) {
        struct fs_sink_ctf_named_field_class *named_fc =
            fs_sink_ctf_field_class_struct_borrow_member_by_index(struct_fc, i);
        ...
        if (fc->type == FS_SINK_CTF_FIELD_CLASS_TYPE_SEQUENCE) {
            ...
            if (seq_fc->length_is_before) {
                ...
                append_integer_field_class_from_props(
                    ctx, 32, 8, false, BT_FIELD_CLASS_INTEGER_PREFERRED_DISPLAY_BASE_DECIMAL, NULL,
                    seq_fc->length_ref->str, true);
            }
             ...
             */
            append_indent(ctx);
            g_string_append(ctx->tsdl, "/* The enumeration and variant field classes "
                                       "below were a trace IR option field class. */\n");
            append_indent(ctx);
            g_string_append(ctx->tsdl, "enum : ");
            append_integer_field_class_from_props(
                ctx, 8, 8, false, BT_FIELD_CLASS_INTEGER_PREFERRED_DISPLAY_BASE_DECIMAL, NULL, NULL,
                false);
            g_string_append(ctx->tsdl, " {\n");
            ctx->indent_level++;
            append_indent(ctx);
            ...
            if (var_fc->tag_is_before) {
                ...
                g_string_append(ctx->tsdl, "enum : ");
                append_integer_field_class_from_props(
                    ctx, 16, 8, false, BT_FIELD_CLASS_INTEGER_PREFERRED_DISPLAY_BASE_DECIMAL, NULL,
                    NULL, false);
                g_string_append(ctx->tsdl, " {\n");
                ctx->indent_level++;
                ...
                for (i = 0; i < var_fc->options->len; i++) {
                    struct fs_sink_ctf_named_field_class *option_named_fc =
                        fs_sink_ctf_field_class_variant_borrow_option_by_index(var_fc, i);

                    append_indent(ctx);
                    g_string_append_printf(ctx->tsdl, "\"%s\" = %" PRIu64 ",\n",
...
static void append_event_class(ctf::sink::CtfIrToTsdlCtx *ctx, struct fs_sink_ctf_event_class *ec)
{
    ...
    if (bt_event_class_get_log_level(ec->ir_ec, &log_level) == BT_PROPERTY_AVAILABILITY_AVAILABLE) {
        unsigned int level;
...
void translate_trace_ctf_ir_to_tsdl(struct fs_sink_ctf_trace *trace, GString *tsdl)
{
    ...
    if (count > 0) {
        ...
        for (i = 0; i < count; i++) {
            ...
            bt_trace_borrow_environment_entry_by_index_const(trace->ir_trace, i, &name, &val);
            append_indent(&ctx);
            g_string_append_printf(tsdl, "%s = ", name);

            switch (bt_value_get_type(val)) {
            case BT_VALUE_TYPE_SIGNED_INTEGER:
                g_string_append_printf(tsdl, "%" PRId64, bt_value_integer_signed_get(val));
                break;
    ...
    for (i = 0; i < trace->stream_classes->len; i++) {
        append_stream_class(&ctx, (fs_sink_ctf_stream_class *) trace->stream_classes->pdata[i]);
    }
}
