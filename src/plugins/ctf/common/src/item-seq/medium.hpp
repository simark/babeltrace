...
public:
     ...
     */
    explicit Buf(const std::uint8_t *addr, bt2_common::DataLen size) noexcept;
     ...
     */
    Buf slice(bt2_common::DataLen offset) const noexcept;

private:
    const std::uint8_t *_mAddr = nullptr;
    bt2_common::DataLen _mSize = bt2_common::DataLen::fromBits(0);
};
...
public:
     ...
     *     User error.
     */
    virtual Buf buf(bt2_common::DataLen offset, bt2_common::DataLen minSize) = 0;
};
