    ...
    _mMedium {std::move(medium)}, _mTraceCls {&traceCls}
{
    ...
    /* Set first item or `_mIsEnded` */
    ++*this;
}

ItemSeqIter::ItemSeqIter(std::unique_ptr<Medium> medium, const TraceCls& traceCls,
                         const bt2c::DataLen pktOffset) :
    ItemSeqIter {std::move(medium), traceCls}
{
...
{
    _mCurClsId = nonstd::nullopt;
    _mLastFixedLenBitArrayFieldByteOrder = nonstd::nullopt;
    _mStack.clear();
    _mDefClkVal = 0;
    ...
    /*
     * Initially not ended: _handleTryBeginReadPktState() could end it
     * immediately.
     */
    _mIsEnded = false;
     ...
     * _requireContentData() call throws `bt2_common::TryAgain`, for
     * example.
     *
...
ItemSeqIter::_StateHandlingReaction ItemSeqIter::_handleBeginReadDynLenBlobFieldState()
{
    ...
    return this->_handleCommonBeginReadStrBlobFieldState(this->_savedUIntVal(blobFc),
                                                         _State::READ_BLOB_FIELD_SECTION, blobFc);
}
