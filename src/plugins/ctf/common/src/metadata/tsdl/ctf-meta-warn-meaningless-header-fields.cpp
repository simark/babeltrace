...
static inline void warn_meaningless_fields(struct ctf_field_class *fc, const char *name,
                                           const char *scope_name, const ctf::LogCfg& logCfg)
{
    uint64_t i;
    ...
    case CTF_FIELD_CLASS_TYPE_STRING:
        warn_meaningless_field(name, scope_name, logCfg);
        break;
    ...
    case CTF_FIELD_CLASS_TYPE_STRUCT:
    {
        ...
        for (i = 0; i < struct_fc->members->len; i++) {
            ...
            warn_meaningless_fields(named_fc->fc, named_fc->name->str, scope_name, logCfg);
        }
    ...
    case CTF_FIELD_CLASS_TYPE_SEQUENCE:
    {
        ...
        warn_meaningless_fields(array_fc->elem_fc, name, scope_name, logCfg);
        break;
    }
    default:
        bt_common_abort();
                                                    ...
                                                    const ctf::LogCfg& logCfg)
{
    ...
    if (!ctf_tc->is_translated) {
        warn_meaningless_fields(ctf_tc->packet_header_fc, NULL, "packet header", logCfg);
    }
    ...
    for (i = 0; i < ctf_tc->stream_classes->len; i++) {
        ...
        if (!sc->is_translated) {
            warn_meaningless_fields(sc->event_header_fc, NULL, "event header", logCfg);
        }
    }
}
