...
bt2c::FileUP Ctf1MetadataStreamParser::_fileUpFromStr(const std::string& str)
{
    ...
    if (!fp) {
        BT_COMP_OR_COMP_CLASS_LOGE_APPEND_CAUSE_AND_THROW(
            bt2c::Error, _mLogCfg.selfComp, _mLogCfg.selfCompClass, "bt_fmemopen() failed.");
    }
...
void Ctf1MetadataStreamParser::_parseSection(const uint8_t * const begin, const uint8_t * const end)
{
    ...
    /* Append the metadata text content to the TSDL scanner */
    {
        ...
        if (ret) {
            BT_COMP_OR_COMP_CLASS_LOGE_APPEND_CAUSE_AND_THROW(
                ...
                "Cannot create the metadata stream AST from TSDL text: ret=%d", ret);
        }
    }
    ...
    /* Make some basic AST node validation */
    {
        ...
        if (ret) {
            BT_COMP_OR_COMP_CLASS_LOGE_APPEND_CAUSE_AND_THROW(
                bt2_common::Error, _mLogCfg.selfComp, _mLogCfg.selfCompClass,
                "Failed to validate metadata stream AST nodes: ret=%d", ret);
        }
    }
