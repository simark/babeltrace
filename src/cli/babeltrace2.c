...
void print_plugin_info(const bt_plugin *plugin)
{
	...
	const bt_plugin_provider_info *provider_info;
	...
	version_avail = bt_plugin_get_version(plugin, &major, &minor,
		&patch, &extra);
	provider_info = bt_plugin_borrow_provider_info(plugin);
	provider_name = provider_info ?
		bt_plugin_provider_info_get_name(provider_info) : NULL;
	printf("%s%s%s%s:\n", bt_common_color_bold(),
...
static
void print_plugin_provider_info(const bt_plugin_provider_info *info)
{
	unsigned int major, minor, patch;
	const char *extra;
	bt_property_availability version_avail;
	...
	name = bt_plugin_provider_info_get_name(info);
	path = bt_plugin_provider_info_get_path(info);
	author = bt_plugin_provider_info_get_author(info);
	license = bt_plugin_provider_info_get_license(info);
	description = bt_plugin_provider_info_get_description(
		info);
	version_avail = bt_plugin_provider_info_get_version(info,
...
enum bt_cmd_status cmd_list_plugin_providers(void)
{
	...
	plugin_providers_count = bt_get_plugin_provider_count();
	...
	for (i = 0; i < plugin_providers_count; i++) {
		const bt_plugin_provider_info * info =
			bt_borrow_plugin_provider_info_by_index(i);

		printf("\n");
		print_plugin_provider_info(info);
	}
