...
{
    return bt2_common::makeUnique<const JsonNullVal>(std::move(loc));
}

JsonBoolVal::UP createJsonVal(const bool val, TextLoc loc)
{
    return bt2_common::makeUnique<const JsonBoolVal>(val, std::move(loc));
}

JsonSIntVal::UP createJsonVal(const long long val, TextLoc loc)
{
    return bt2_common::makeUnique<const JsonSIntVal>(val, std::move(loc));
}

JsonUIntVal::UP createJsonVal(const unsigned long long val, TextLoc loc)
{
    return bt2_common::makeUnique<const JsonUIntVal>(val, std::move(loc));
}

JsonRealVal::UP createJsonVal(const double val, TextLoc loc)
{
    return bt2_common::makeUnique<const JsonRealVal>(val, std::move(loc));
}

JsonStrVal::UP createJsonVal(std::string val, TextLoc loc)
{
    return bt2_common::makeUnique<const JsonStrVal>(std::move(val), std::move(loc));
}

JsonArrayVal::UP createJsonVal(JsonArrayVal::Container&& vals, TextLoc loc)
{
    return bt2_common::makeUnique<const JsonArrayVal>(std::move(vals), std::move(loc));
}

JsonObjVal::UP createJsonVal(JsonObjVal::Container&& vals, TextLoc loc)
{
    return bt2_common::makeUnique<const JsonObjVal>(std::move(vals), std::move(loc));
}
