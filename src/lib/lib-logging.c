...
static inline void format_blob_field_class(char **buf_ch,
		bool extended, const char *prefix,
		...
		const char *prefix, const struct bt_field_class *field_class)
{
	...
	case BT_FIELD_CLASS_TYPE_STATIC_BLOB:
	{
		...
		format_blob_field_class(buf_ch, extended, prefix,
			(const void *) blob_fc);
		...
		const char *prefix, const struct bt_event_class *event_class)
{
	...
	if (event_class->ns.value) {
		BUF_APPEND(", %snamespace=\"%s\"",
			PRFIELD(event_class->ns.value));
	}
