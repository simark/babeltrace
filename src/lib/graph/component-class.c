...
void destroy_component_class(struct bt_object *obj)
{
	...
	if (class->destruction_listeners) {
		...
		/*
		* The component class' reference count is 0 if we're here.
		...
		* bt_object_get_ref() to be called) on the component class (ref.
...
{
	BT_ASSERT(listener_id < cc->destruction_listeners->len);
	return (&bt_g_array_index(cc->destruction_listeners,
			struct bt_component_class_destruction_listener_elem,
			listener_id))->func;
}
		...
		const struct bt_component_class *_cc, bt_listener_id listener_id)
{
	...
	BT_ASSERT_PRE("listener-id-exists",
		...
		"Trace class has no such component class destruction listener ID: "
