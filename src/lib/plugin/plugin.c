		...
		struct bt_plugin *plugin)
{
	...
	BT_ASSERT_PRE("plugin-is-not-part-of-plugin-set",
		!bt_plugin_set_borrow_plugin_by_name_const(
			plugin_set, plugin->info.name->str),
		"Plugin is already part of the plugin set: "
...
{
	BT_ASSERT(listener_id < plugin->destruction_listeners->len);
	return (&bt_g_array_index(plugin->destruction_listeners,
			struct bt_plugin_destruction_listener_elem,
			listener_id))->func;
}
		...
		const struct bt_plugin *_plugin, bt_listener_id listener_id)
{
	...
	BT_ASSERT_PRE("listener-id-exists",
		...
		"Trace class has no such plugin destruction listener ID: "
