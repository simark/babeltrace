...
<ol>
  ...
  <li>
    ...
    plugin provider's name is a valid C identifier, or with
    ...
    @note
        When you use BT_PLUGIN_PROVIDER(), the plugin provider's ID is
        <code>auto</code>.
  </li>
...
@code
BT_PLUGIN_PROVIDER_AUTHOR_WITH_ID(my_plugin_provider_id, "Patrick Bouchard");
@endcode
...
@note
    ...
    BT_PLUGIN_PROVIDER_AUTHOR("Patrick Bouchard");
    @endcode
    @endparblock
...
@param[in] _id
    ...
    Plugin provider's ID, unique amongst all the plugin provider IDs of
...
@param[in] _major
    ...
    Plugin provider's major version.
    @endparblock
@param[in] _minor
    @parblock
    <code>unsigned int</code>

    Plugin provider's minor version.
    @endparblock
@param[in] _patch
    @parblock
    <code>unsigned int</code>

    Plugin provider's patch version.
    @endparblock
@param[in] _extra
    @parblock
    <code>const char *</code>

    Plugin provider's version's extra information.

    Can be \c NULL if the plugin provider's version has no extra information.
    @endparblock
*/
...
@param[in] _func
    ...
    Plugin provider's initialization function.
    @endparblock
...
@param[in] _func
    ...
    Plugin provider's finalization function.
    @endparblock
...
@brief
    ...
    setting \bt_p{*plugin_set_out} to the result.

If any plugin loading error occurs during this function's execution, the
function, depending on the result of
bt_plugin_provider_create_all_from_file_options_get_fail_on_load_error()
called with  with \bt_p{options}, will:

<dl>
  <dt>#BT_TRUE</dt>
  <dd>Returns #BT_PLUGIN_PROVIDER_CREATE_ALL_FROM_FILE_FUNC_STATUS_ERROR.</dd>
...
#BT_PLUGIN_PROVIDER_CREATE_ALL_FROM_FILE_FUNC_STATUS_NOT_FOUND and does
\em not set \bt_p{*plugins}.
...
@param[in] options
    ...
    @endparblock
@param[out] plugin_set_out
    <strong>On success</strong>, \bt_p{*plugin_set_out} is a new plugin
    set reference which contains all the plugins found in the file with
    path \bt_p{path}.
...
@bt_pre_not_null{options}
@bt_pre_not_null{plugin_set_out}
*/
...
@param[in] _func
    ...
    Plugin provider's create all from file function.
    @endparblock
...
<dl>
  <dt>#BT_TRUE</dt>
  <dd>Returns #BT_PLUGIN_PROVIDER_CREATE_ALL_FROM_STATIC_FUNC_STATUS_ERROR.</dd>
...
If this function doesn't find any plugin, it returns
#BT_PLUGIN_PROVIDER_CREATE_ALL_FROM_STATIC_FUNC_STATUS_NOT_FOUND and does
\em not set \bt_p{*plugins}.
...
@param[out] plugin_set_out
    <strong>On success</strong>, \bt_p{*plugin_set_out} is a new plugin
    set reference which contains all the static plugins.
...
// FIXME: doc
// FIXME: ensure all new functions have __BT_NOEXCEPT
...
@param[in] _func
    ...
    Plugin provider's create all from static function.
    @endparblock
